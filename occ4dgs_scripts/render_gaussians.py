"""
scripts/render_gaussians.py

Visualizes the raw GAUSSIAN PRIMITIVES themselves (not occupancy predictions)
for do-nothing / Dynamic 3DGS (L=2) / Static 3DGS, plus a 4th panel showing
per-Gaussian displacement magnitude (do-nothing -> deformed) -- a genuinely
different visualization than render_qualitative.py's occupancy comparisons,
which show splatted/rasterized predictions at fixed evaluation grid points,
not the underlying Gaussians (25,600 points here, vs 640,000 evaluation
points there).

Raw Gaussian semantic class mapping: NOT independently confirmed via
exhaustive source review (see conversation) -- reasoned assumption is
index 0-15 -> class labels 1-16, index 16 -> empty. Flagged as such; check
whether driveable_surface/vegetation appear in geometrically plausible
...
OUT_DIR = os.path.join(STAGEB_DIR, "gaussian_data_L2")

N_G = 25600
EMBED_DIMS = 128
NUM_BLOCKS = 2

SCENE_INDICES = [0, 20, 45, 70, 95, 120, 10, 35, 60, 85, 110, 135]
...
def main():
    ...
    print("Loading Dynamic 3DGS (L=2) checkpoint...")
    ...
    with torch.no_grad():
        for scene_idx in SCENE_INDICES:
            ...
            donothing_means = mu_proj_clamped
            donothing_class = g0_semantics.argmax(-1)
            ...
            deformed_means = g_1.means
            deformed_class = g_1.semantics.argmax(-1)
            ...
            static_gaussian = representation_oracle[-1]["gaussian"]
            static_means = static_gaussian.means[0]
            static_class = static_gaussian.semantics[0].argmax(-1)
            torch.cuda.empty_cache()

            displacement = (deformed_means - donothing_means).norm(dim=-1)
            ...
            np.savez(
                ...
                donothing_means=donothing_means.cpu().numpy(),
                donothing_class=donothing_class.cpu().numpy(),
                deformed_means=deformed_means.cpu().numpy(),
                deformed_class=deformed_class.cpu().numpy(),
                static_means=static_means.cpu().numpy(),
                static_class=static_class.cpu().numpy(),
                displacement=displacement.cpu().numpy(),
            )
            print(f"  Saved -> {out_path} ({len(donothing_means)} Gaussians, "
                  f"mean displacement {displacement.mean().item():.3f}m)")
