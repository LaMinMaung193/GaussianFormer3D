...
def main(local_rank, args):
    ...
    if args.gpus > 1:
        ...
        port = os.environ.get("MASTER_PORT", "29400")
    ...
    if local_rank == 0:
        wandb.init(project='GaussianFusion', config={"lr": cfg.optimizer["optimizer"]["lr"], "dataset": "nuScenes", "epochs": cfg.max_epochs}, name=args.work_dir.split('/')[-1])
    ...
    while epoch < max_num_epochs:
        ...
        with torch.no_grad():
            for i_iter_val, data in enumerate(val_dataset_loader):
                ...
                for idx, pred in enumerate(result_dict['pred_occ'][-1]):
                    ...
                    if 'occ3d_mask_camera' in data:
                        occ3d_mask_camera = data.pop('occ3d_mask_camera').flatten(1)[idx]
        ...
        if local_rank == 0:
            wandb.log({'epoch':epoch, 'val_loss': np.mean(val_loss_list), 'mIoU': miou, 'iou2': iou2})
        miou_metric.reset()
